...
const GuideModal = ({ isOpen, setIsOpen, getBlogs, mode, singleBlog }) => {
  const toggleModal = () => {
    setIsOpen((prev) => !prev);
  };
  ...
  useEffect(() => {
    if (mode === "update") {
      setFormState({
        ...
        // content: JSON.parse(singleBlog.content.blocks.text),
      });
    }
  }, [singleBlog]);
  ...
  const handleSubmitUpdate = async (e) => {
    e.preventDefault();
    const { id, title, description, content } = formState;
    const formData = new FormData();
  ...
  return (
    <div>
      {/* Modal */}
      {isOpen && (
        <div className="fixed inset-0 flex items-start justify-center p-5 overflow-y-auto z-50 bg-black bg-opacity-50">
          <div className="bg-white p-5 rounded-lg shadow-lg w-full max-w-lg relative">
            ...
            <h2 className="text-xl font-bold mb-5">
              {mode === "update" ? "Update Guide" : "Create Guide"}
            </h2>
            <form
              onSubmit={mode === "update" ? handleSubmitUpdate : handleSubmit}
              className="space-y-4"
            >
              <div>
